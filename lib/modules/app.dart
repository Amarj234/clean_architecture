  ...
  Widget build(BuildContext context) {
    return MediaQuery(
      data: MediaQuery.of(context).copyWith(
        textScaler:
            const TextScaler.linear(1.0), // Disable scaling for this section
      ),
      ...
      return BlocBuilder<ThemchangeBloc, ThemchangeState>(
        ...
        builder: (context, state2) {
          return MaterialApp(
            ...
            highContrastTheme: MyThemes.getThemeData(context1,
                seedColor: state.currentTheme?.color.fromHexToColor() ??
                    AppColors.primaryColor,
                themval: state2.themval!),
            themeMode: ThemeMode.dark,
            theme: MyThemes.getThemeData(context1,
                seedColor: state.currentTheme?.color.fromHexToColor() ??
                    AppColors.primaryColor,
                themval: state2.themval!),
                ...
                listener: (context1, state) {
                  switch (state.status) {
                    case AuthenticationStatus.authenticated:
                      context
                          .read<AppDataBloc>()
                          .add(const AppDataEvent.getUser());
                ...
                child: MediaQuery(
                  data: MediaQuery.of(context).copyWith(
                    textScaler: const TextScaler.linear(
                        1.0), // Disable text scaling globally
                  ),
                  child: child!,
                ),
              );
